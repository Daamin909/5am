...
def build_stats_text(week_days, month_days,):
    stats_text = (
        f"*Early Morning Message Stats*\n"
        f"• This week: *{len(week_days)}* day(s)\n"
        f"• This month: *{len(month_days)}* day(s)\n"
        f"*Dates (Month)*:\n"
    )
    if month_days:
...
def post_stats_if_early():
    try:
        ...
        if not early_today:
            main_text = "<@U07GLQY6UN4> dumbahh couldn't even wake up before 5 :loll:"
        ...
        main_text = "<@U07GLQY6UN4> bro thinks he tuff by waking up before 5 am :icant:"
...
if __name__ == "__main__":
    if TEST_MODE:
        ...
        ist_scheduler_loop(target_hour=10, target_minute=53)
